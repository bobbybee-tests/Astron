...
void MessageDirector::subscribe_channel(MDParticipantInterface* p, channel_t c)
{
	...
	if(is_client)
	{
		...
		DatagramPtr dg = Datagram::create_control(CONTROL_ADD_CHANNEL);
		dg->add_channel(c);
		send_datagram(dg);
	}
}
...
void MessageDirector::unsubscribe_channel(MDParticipantInterface* p, channel_t c)
{
	...
	if(is_client)
	{
		...
		// Send upstream control message
		DatagramPtr dg = Datagram::create_control(CONTROL_REMOVE_CHANNEL);
		dg->add_channel(c);
		send_datagram(dg);
	}
}
...
void MessageDirector::subscribe_range(MDParticipantInterface* p, channel_t lo, channel_t hi)
{
	...
	if(is_client)
	{
		...
		// Send upstream control message
		DatagramPtr dg = Datagram::create_control(CONTROL_ADD_RANGE);
		dg->add_channel(lo);
		dg->add_channel(hi);
		send_datagram(dg);
	}
...
void MessageDirector::unsubscribe_range(MDParticipantInterface *p, channel_t lo, channel_t hi)
{
	...
	if(is_client)
	{
		...
		for(auto it = silent_intervals.begin(); it != silent_intervals.end(); ++it)
		{
			...
			DatagramPtr dg = Datagram::create_control(CONTROL_REMOVE_RANGE);
			dg->add_channel(lo);
			dg->add_channel(hi);
			send_datagram(dg);
		}
